...
- [x] web 5173 + express 3001 + nest 3002 одночасно — підтверджено власником
- [x] Браузер: логін — підтверджено власником
- [x] Браузер: перемикач бекендів — перевірено в Chrome (запити йдуть на
      :3001 / :3002 залежно від перемикача) + власником
- [x] Логін через express → перемкнути на nest → `/api/auth/me` 200 з тією ж
      cookie — curl (обидві схеми: з проксі і прямі запити) + власником
...
- [x] Фінальний стан цього чекліста (2026-09-23)

## Після міграції (поза початковим планом)
- [x] Проєкти в `~/Documents/programing/auto-lincoln/`, `test-pr` → `auto-lincoln-web`
- [x] Прямі запити з браузера на :3001 / :3002 замість Vite-проксі, CORS в обох API
- [x] Тестовий акаунт `test@autolincoln.local` (manager) у seed
- [x] Чотири репо запушено на GitHub (власником)
- [x] `auto-lincoln/context/` — контекст для Claude Project у браузері
